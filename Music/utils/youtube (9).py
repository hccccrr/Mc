...
class YouTube:
    ...
    async def send_song(
        ...
        try:
            ...
            await hellbot.logit(
                ...
                f"**🎵 Track:** [{track['title']}]({track['link']})",
            )
            ...
            LOGS.error(f"❌ Send song error: {e}")
    ...
    async def get_lyrics(self, song: str, artist: str = "") -> dict:
        ...
        if not self.client:
            LOGS.warning("⚠️ Lyrics API not available")
            return context
        ...
        try:
            ...
            if results:
                ...
                context = {
                    "title": results_dict.get("full_title", song),
                    "image": results_dict.get("song_art_image_url", ""),
                    "lyrics": results_dict.get("lyrics", "Lyrics not found"),
                }
                
                LOGS.info(f"✅ Found lyrics for: {song}")
        
        except Exception as e:
            LOGS.error(f"❌ Lyrics error: {e}")
